...
static SPI_HandleTypeDef *_sd_phspi;
...
static void SPI_TxByte(BYTE data)
{
	...
	while (HAL_SPI_GetState(_sd_phspi) != HAL_SPI_STATE_READY);
	HAL_SPI_Transmit(_sd_phspi, &data, 1, SPI_TIMEOUT);
...
static uint8_t SPI_RxByte(void)
{
  ...
  while ((HAL_SPI_GetState(_sd_phspi) != HAL_SPI_STATE_READY));
  HAL_SPI_TransmitReceive(_sd_phspi, &dummy, &data, 1, SPI_TIMEOUT);
//  while ((HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY));
...
//  Timer1 = 10;
  ...
  } while((token == 0xFF) && ((HAL_GetTick() - tickstart) < 100));
...
//			if (!Timer1 || SD_SendCmd(CMD16, 512) != 0){
			if (!( (HAL_GetTick() - tickstart) < 1000 ) || SD_SendCmd(CMD16, 512) != 0){
				/* 블럭 길이 선택 */
				type = 0;
			}
		}
